...
public class Proveedor {
	...
	public String validate() {
		...
		} else if (!isValid("^[a-zA-ZÀ-ÿ\\s]{8,40}$", this.ciudadProveedor, 40)) {
			return "Nombre de ciudad inválido, sólo se permiten entre 8 y 40 caracteres";
		} else if (!isValid("^[a-zA-Z0-9!@#$%^&.*\\s]{8,100}$", this.direccionProveedor, 100)) {
			return "La dirección debe contener entre 8 y 100 caracteres máximo";
		} else if (!isValid("^[a-zA-Z0-9_-\\s]{4,40}$", this.telefonoProveedor, 40)) {
			return "El teléfono debe contener entre 4 y 40 caracteres.";
		} else if (!isValid("^[a-zA-ZÀ-ÿ\\s]{8,40}$", this.nombreProveedor, 40)) {
